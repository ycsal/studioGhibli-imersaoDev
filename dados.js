const dados = [
    {
      titulo: 'A Viagem de Chihiro',
      ano: 2001,
      classificacao: 'Classificação Livre',
      genero: 'Fantasia',
      sinopse: 'Chihiro, uma menina de 10 anos, se perde em um mundo espiritual ao lado de seus pais. Para voltar para casa, ela precisa trabalhar em uma casa de banho para espíritos.',
      plataformas: 'Netflix, HBO Max',
      tags: 'espiritualidade banho espíritos espiritos onirico espirito espírito onírico viagem amadurecimento'
    },
    {
      titulo: 'Meu Amigo Totoro',
      ano: 1988,
      classificacao: 'Classificação Livre',
      genero: 'Fantasia',
      sinopse: 'Duas irmãs se mudam para o campo com o pai e descobrem um mundo mágico habitado por criaturas encantadoras, como o Totoro.',
      plataformas: 'Disney+',
      tags: 'natureza infância infancia magia amigos amigo amizade campo'
    },
    {
      titulo: 'A Princesa Mononoke',
      ano: 1997,
      classificacao: 'Classificação +12 anos',
      genero: 'Aventura, Fantasia',
      sinopse: 'Um jovem príncipe se envolve em uma guerra entre deuses e homens, lutando para encontrar um equilíbrio entre a natureza e a civilização.',
      plataformas: 'Netflix',
      tags: 'natureza ecologia conflito deuses animais'
    },
    {
      titulo: 'O Castelo Animado',
      ano: 2004,
      classificacao: 'Classificação +10 anos',
      genero: 'Fantasia',
      sinopse: 'Sophie, uma jovem chapeleira, é amaldiçoada por uma bruxa e transformada em uma velha. Ela busca a ajuda de um misterioso castelo para reverter a magia.',
      plataformas: 'Netflix',
      tags: 'magia amor transformação transformacao castelo bruxaria'      
    },
    {
      titulo: 'O Serviço de Entregas da Kiki',
      ano: 1989,
      classificacao: 'Classificação Livre',
      genero: 'Fantasia',
      sinopse: 'Uma jovem bruxa, Kiki, sai de casa para completar seu treinamento e aprender a voar com sua vassoura.',
      plataformas: 'Netflix, Apple TV',
      tags: 'independência independencia bruxa amigo amigos bruxaria amizade amadurecimento cidade'
    },
    {
      titulo: 'Pom Poko',
      ano: 1994,
      classificacao: 'Classificação +10 anos',
      genero: 'Comédia, Fantasia',
      sinopse: 'Um grupo de guaxinins luta para proteger sua floresta da urbanização crescente.',
      plataformas: 'Netflix',
      tags: 'natureza preservacao comedia urbanizacao preservação animais engracado engraçado comédia urbanização'
    },
    {
      titulo: 'O Conto da Princesa Kaguya',
      ano: 2013,
      classificacao: 'Classificação Livre',
      genero: 'Drama, Fantasia',
      sinopse: 'A história de uma princesa encontrada dentro de um bambu, que cresce rapidamente e enfrenta os desafios da vida na corte imperial.',
      plataformas: 'Netflix, Apple TV',
      tags: 'mitologia tradição tradicao familia crescer crescimento família destino'
    },
    {
      titulo: 'Vidas ao Vento',
      ano: 2013,
      classificacao: 'Classificação +12 anos',
      genero: 'Drama, Histórico, Guerra',
      sinopse: 'A história do engenheiro Jiro Horikoshi, criador do caça Zero, e seu amor por Naoko Satomi em meio à Segunda Guerra Mundial.',
      plataformas: 'Netflix, Apple TV',
      tags: 'história historia aviacao guerra sonho aviação amor'
    },
    {
      titulo: 'O Menino e a Garça',
      ano: 2023,
      classificacao: 'Classificação Livre',
      genero: 'Animação, Fantasia, Aventura',
      sinopse: 'Em uma pequena vila à beira-mar, um menino solitário encontra uma garça ferida e a cuida até que ela se recupere. Juntos, embarcam em uma jornada mágica e emocionante, descobrindo um mundo secreto e cheio de maravilhas.',
      plataformas: 'Até o momento, indisponível em plataformas de streaming',
      tags: 'amizade animal natureza aventura fantasia mar magia'
    },
    {
      titulo: 'Túmulo dos Vagalumes',
      ano: 1988,
      classificacao: 'Classificação +12 anos',
      genero: 'Drama, Histórico, Guerra',
      sinopse: 'Em meio aos horrores da Segunda Guerra Mundial, Seita e Setsuko, irmãos órfãos, lutam para sobreviver em uma cidade japonesa devastada pelos bombardeios. A animação retrata de forma comovente a luta pela sobrevivência e a inocência perdida na guerra.',
      plataformas: 'Netflix',
      tags: 'guerra tumulo historia história crianças família perda sofrimento clássico'
    },
    {
      titulo: 'Ponyo - Uma Amizade que Veio do Mar',
      ano: 2008,
      classificacao: 'Livre',
      genero: 'Fantasia, Aventura',
      sinopse: 'Ponyo, uma jovem sereia, sonha em se tornar humana. Após ser salva por um menino chamado Sosuke, ela embarca em uma aventura mágica que ameaça o equilíbrio entre o mar e a terra.',
      plataformas: 'Netflix, Disney+',
      tags: 'sereia mar magia amizade aventura fantasia amigos'
    }
  ];
